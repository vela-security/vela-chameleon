...
import (
	...
	binlogdatapb "github.com/vela-security/vela-chameleon/vitess/go/vt/proto/binlogdata"
)

// BinlogEvent represents a single vela-event from a raw MySQL binlog dump stream.
...
type BinlogEvent interface {
	...
	// vela-event. This should be called first on any BinlogEvent, and other
	...
	IsRand() bool
	// IsPreviousGTIDs returns true if this vela-event is a PREVIOUS_GTIDS_EVENT.
	...
	// Timestamp returns the timestamp from the vela-event header.
	Timestamp() uint32

	// Format returns a BinlogFormat struct based on the vela-event data.
	// This is only valid if IsFormatDescription() returns true.
	Format() (BinlogFormat, error)
	// GTID returns the GTID from the vela-event, and if this vela-event
	...
	// PreviousGTIDs returns the Position from the vela-event.
	...
	// WriteRows or DeleteRows vela-event.
	...
	// StripChecksum returns the checksum and a modified vela-event with the
	// checksum stripped off, if any. If there is no checksum, it returns
	// the same vela-event and a nil checksum.
...
// This structure is passed to subsequent vela-event types to let them know how to
// parse themselves.
type BinlogFormat struct {
	...
	// HeaderLength is the size in bytes of vela-event headers other
...
// HeaderSize returns the header size of any vela-event type.
...
type Rows struct {
	// Flags has the flags from the vela-event.
	Flags uint16
	...
	// Rows is an array of Row in the vela-event.
	Rows []Row
}

// Row contains data for a single Row in a Rows vela-event.
type Row struct {
