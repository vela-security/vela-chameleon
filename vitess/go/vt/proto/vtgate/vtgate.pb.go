...
import (
	...
	binlogdata "github.com/vela-security/vela-chameleon/vitess/go/vt/proto/binlogdata"
	query "github.com/vela-security/vela-chameleon/vitess/go/vt/proto/query"
	topodata "github.com/vela-security/vela-chameleon/vitess/go/vt/proto/topodata"
	vtrpc "github.com/vela-security/vela-chameleon/vitess/go/vt/proto/vtrpc"
)
...
type UpdateStreamRequest struct {
	...
	// unused is vela-event is set, and we are only streaming from the shard
	// described by vela-event.shard.
	Timestamp int64 `protobuf:"varint,6,opt,name=timestamp,proto3" json:"timestamp,omitempty"`
	// vela-event is the vela-event to start the stream from.
	// Note it is only used if we are streaming from exactly the same shard
	// as this vela-event was coming from. Otherwise we can't use this vela-event,
	// and will use the timestamp as a starting point.
	Event                *query.EventToken `protobuf:"bytes,7,opt,name=vela-event,proto3" json:"vela-event,omitempty"`
	XXX_NoUnkeyedLiteral struct{}          `json:"-"`
	XXX_unrecognized     []byte            `json:"-"`
	XXX_sizecache        int32             `json:"-"`
}
...
type UpdateStreamResponse struct {
	// vela-event is one vela-event from the stream.
	Event *query.StreamEvent `protobuf:"bytes,1,opt,name=vela-event,proto3" json:"vela-event,omitempty"`
	...
	// shard, this is equal to vela-event.timestamp. If the Update Stream
