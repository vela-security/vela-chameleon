...
import (
	...
	sqle "github.com/vela-security/vela-chameleon/mysql/engine"
	"github.com/vela-security/vela-chameleon/mysql/internal/sockstate"
	"github.com/vela-security/vela-chameleon/mysql/sql"
	"github.com/vela-security/vela-chameleon/mysql/sql/expression"
	"github.com/vela-security/vela-chameleon/mysql/sql/parse"
	"github.com/vela-security/vela-chameleon/mysql/sql/plan"
	"github.com/vela-security/vela-chameleon/vitess/go/mysql"
	"github.com/vela-security/vela-chameleon/vitess/go/netutil"
	"github.com/vela-security/vela-chameleon/vitess/go/sqltypes"
	"github.com/vela-security/vela-chameleon/vitess/go/vt/proto/query"
)
...
func (h *Handler) doQuery(
	...
	if len(bindings) > 0 {
		...
		if err != nil {
			assert.GxEnv().Errorf("Error processing bindings for query %s: %s", query, err)
			return err
		}
	}
	...
	if err != nil {
		assert.GxEnv().Errorf("connection %d: error running query: %s", c.ConnectionID, err)
		return err
	}
...
rowLoop:
	for {
		...
		case err = <-errChan:
			...
			assert.GxEnv().Debugf("connection %d: got error %s", c.ConnectionID, err.Error())
			close(quit)
			return err
		case row := <-rowChan:
			...
			assert.GxEnv().Debugf("connection %d spooling result row %s", c.ConnectionID, outputRow)
			r.Rows = append(r.Rows, outputRow)
			r.RowsAffected++
		case <-timer.C:
			if h.readTimeout != 0 {
				// Cancel and return so Vitess can call the CloseConnection callback
				assert.GxEnv().Debugf("connection %d got timeout", c.ConnectionID)
				close(quit)
	...
	switch len(r.Rows) {
	case 0:
		assert.GxEnv().Debugf("connection %d returning empty result", c.ConnectionID)
	case 1:
		assert.GxEnv().Debugf("connection %d returning result %v", c.ConnectionID, r)
	}
...
func (h *Handler) pollForClosedConnection(c *mysql.Conn, errChan chan error, quit chan struct{}) {
	...
	if !ok {
		assert.GxEnv().Debug("Connection checker exiting, connection isn't TCP")
		return
	}
	...
	if err != nil || inode == 0 {
		if !sockstate.ErrSocketCheckNotImplemented.Is(err) {
			assert.GxEnv().Info("Connection checker exiting, connection isn't TCP")
		}
		return
	}
	...
	if !ok {
		assert.GxEnv().Debug("Connection checker exiting, could not get local port")
		return
	}
	...
	for {
		...
		case sockstate.Broken:
			assert.GxEnv().Info("socket state is broken, returning error")
			errChan <- ErrConnectionWasClosed.New()
			return
		case sockstate.Error:
			assert.GxEnv().Infof("Connection checker exiting, got err checking sockstate: %v", err)
			return
		default: // Established
...
func (h *Handler) handleKill(conn *mysql.Conn, query string) (bool, error) {
	...
	assert.GxEnv().Debugf("killing query %s", query)
